...
public class EventExecutor extends BAExecutor{
	...
	public boolean eventOptions(CommandSender sender, Command cmd, String commandLabel, Object[] args) {
	...
	public boolean eventCancel(CommandSender sender, Command cmd, String commandLabel, Object[] args) {
		if (!event.isRunning() && !event.isOpen()){
			return sendMessage(sender,"&eA "+event.getCommand()+" is not running");
		}
		boolean silent = args.length >1 && ((String) args[1]).equalsIgnoreCase("silent");
		event.cancelEvent();
	...
	public boolean eventStart(CommandSender sender,Command cmd, String commandLabel, Object[] args) {
		...
		boolean forceStart = args.length > 1 && ((String)args[1]).equalsIgnoreCase("force");
	...
	public boolean eventInfo(CommandSender sender, Command cmd, String commandLabel, Object[] args){
	...
	public boolean eventLeave(CommandSender sender,Command cmd, String commandLabel, Object[] args) {
		Player p = (Player) sender;
		if (!event.waitingToJoin(p) && !event.hasPlayer(p)){
			return sendMessage(sender,"&eYou aren't inside the &6" + event.getName());}
		if (!event.canLeave(p)){
			return sendMessage(sender,"&eYou can't leave the &6"+event.getCommand()+"&e while its "+event.getState());}
		event.leave(p);
		return sendMessage(sender,"&eYou have left the &6" + event.getName());
	}

	@MCCommand(cmds={"check"},inGame=true,usage="check", order=2)
	public boolean eventCheck(CommandSender sender, Command cmd, String commandLabel, Object[] args) {
		if (!event.isOpen()){
	...
	public boolean eventJoin(CommandSender sender, Command cmd, String commandLabel, Object[] args) {
		Player p = (Player) sender;
		if (!(p.hasPermission("arena."+event.getCommand().toLowerCase()+".join"))){
			return sendMessage(sender, "&eYou don't have permission to join a &6" + event.getCommand());}
		if (!event.canJoin()){
			return sendMessage(sender,"&eYou can't join the &6"+event.getCommand()+"&e while its "+event.getState());}
		if (!canJoin(sender,p)){
			return true;
		}
		if (event.waitingToJoin(p)){
			return sendMessage(sender,"&eYou have already joined the and will enter when you get matched up with a team");			
		}
		...
		if(!tops.playerReady(p)){
			...
			return MessageController.sendMessage(sender,notReadyMsg);
		}
...
//		System.out.println(" t size=" + sq.getTeamSizeRange() +"   in team = " + t +"   ");
		...
		if (sq.getMaxTeamSize() < t.size()){
			return sendMessage(sender,"&cThis bukkitEvent can only support inEvent up to &6" + sq.getSize()+"&e your team has &6"+t.size());}
		...
		case ADDED:
			...
			break;
		case CANT_FIT:
			sendMessage(sender,"&cThe &6" + event.getDetailedName()+"&c is full");
			break;
		case WAITING_FOR_PLAYERS:
			final int remaining = ar.n;
			sendMessage(sender,"&eYou have joined the &6" + event.getDetailedName());
			sendMessage(sender,"&eYou will enter the bukkitEvent when &6" +remaining+"&e more "+MessageUtil.playerOrPlayers(remaining)+
	...
	public boolean eventStatus(CommandSender sender, Command cmd, String commandLabel, Object[] args) {
	...
	public boolean eventResult(CommandSender sender, Command cmd, String commandLabel, Object[] args) {
